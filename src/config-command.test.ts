...
describe("handleConfigCommand", () => {
  ...
  it("sets default model", async () => {
    ...
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("haiku"), "info");
  });

  it("opens model picker when set-default has no model", async () => {
    vi.mocked(selectModel).mockResolvedValue({ provider: "anthropic", id: "claude-haiku", name: "Haiku" } as any);
    const ctx = mockCtx();
    await handleConfigCommand("set-default", ctx);
    expect(selectModel).toHaveBeenCalled();
    expect(setDefaultModel).toHaveBeenCalledWith("/cwd", "anthropic/claude-haiku");
  });
  ...
  it("sets per-lens model", async () => {
    ...
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("sonnet"), "info");
  });

  it("rejects invalid lens", async () => {
    const ctx = mockCtx();
    await handleConfigCommand("set-lens invalid-model sonnet", ctx);
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Invalid lens"), "error");
  ...
  it("toggles interactive on", async () => {
    ...
    expect(saveConfig).toHaveBeenCalledWith("/cwd", expect.objectContaining({ interactive: true }));
  });
  ...
  it("toggles interactive off", async () => {
    ...
    expect(saveConfig).toHaveBeenCalledWith("/cwd", expect.objectContaining({ interactive: false }));
  });
  ...
  it("toggles confirm on", async () => {
    ...
    expect(saveConfig).toHaveBeenCalledWith("/cwd", expect.objectContaining({ confirmBeforeRun: true }));
  });
  ...
  it("toggles confirm off", async () => {
    ...
    expect(saveConfig).toHaveBeenCalledWith("/cwd", expect.objectContaining({ confirmBeforeRun: false }));
  });
  ...
  it("sets context-mode to diff", async () => {
    ...
    expect(saveConfig).toHaveBeenCalledWith("/cwd", expect.objectContaining({ contextMode: "diff" }));
  });
  ...
  it("sets context-mode to full", async () => {
    ...
    expect(saveConfig).toHaveBeenCalledWith("/cwd", expect.objectContaining({ contextMode: "full" }));
  });
  ...
  it("resets prompts", async () => {
    ...
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("regenerated"), "info");
  });

  it("reports unknown subcommand", async () => {
    const ctx = mockCtx();
    await handleConfigCommand("unknown-thing", ctx);
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Unknown subcommand"), "error");
  });
});
