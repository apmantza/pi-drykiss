...
export async function resolveModelSmart(
  ...
  if (config.interactive !== false && ctx.hasUI) {
    const selected = await selectModel(
      ctx,
      "Select Model",
      lens ? `Choose a model for the ${lens} review` : "Choose a model for the review",
    );
    if (selected) {
      // Save to config so they don't have to pick again
      if (lens) {
        config.lensModels = { ...config.lensModels, [lens]: `${selected.provider}/${selected.id}` };
      } else {
        config.defaultModel = `${selected.provider}/${selected.id}`;
      }
      await saveConfig(cwd, config);
      ctx.ui.notify(`Saved ${selected.name} as default for ${lens ?? "all reviews"}.`, "info");
      return selected;
    }
  }
...
export async function callLLM(
  ...
  if (!model) {
    throw new Error(
      "No model available. Configure an API key with /login.",
    );
  }
