...
import { parseArgs } from "./review-command.js";
...
describe("parseArgs", () => {
  ...
  it("parses file paths", () => {
    const opts = parseArgs("src/foo.ts src/bar.ts");
    expect(opts.files).toEqual(["src/foo.ts", "src/bar.ts"]);
  });

  it("parses mixed args", () => {
    const opts = parseArgs("--staged src/foo.ts --ref=develop src/bar.ts");
    expect(opts.staged).toBe(true);
  ...
  it("parses --model flag", () => {
    const opts = parseArgs("--model=sonnet src/foo.ts");
    expect(opts.model).toBe("sonnet");
    expect(opts.files).toEqual(["src/foo.ts"]);
  });
  ...
  it("parses all flags together", () => {
    const opts = parseArgs("--staged --ref=main --model=haiku src/a.ts src/b.ts");
    expect(opts.staged).toBe(true);
    expect(opts.ref).toBe("main");
...
describe("edit-tracker", () => {
  ...
  it("tracks multiple files in one turn", () => {
    ...
    expect(edits.files.map((f) => f.language)).toEqual(["TypeScript", "Python", "Go"]);
  });
